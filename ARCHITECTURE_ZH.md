...
1. `preload_assets(source)`（`crates/opencat-web/src/resource/wasm_api.rs:65`）：
   ...
   - 通过 `build_catalog()` 探测元数据
   - 递归扫描 Lottie 依赖
2. 构建字体数据库（默认 NotoSansSC + NotoColorEmoji + 文档字体）
3. 使用字体数据库解析源码
4. `collect_resource_requests_from_parsed()`
5. 从 `BlobStore` 字节构建 `build_catalog()`
6. 从获取的 SRT 做 `hydrate_captions()`
...
Host： 获取字节 → 构建 Catalog → 字幕 Hydration → 字体数据库 → [Pipeline] → 解码 → 执行
```

Core 从不做 I/O。它声明需要的资源（`ResourceRequests`），从字节探测元数据（`build_catalog`），产生确定性的绘制指令（`DrawOpFrame`）。Host（engine Skia、web CanvasKit）各自独立实现资源获取、解码和像素输出。
