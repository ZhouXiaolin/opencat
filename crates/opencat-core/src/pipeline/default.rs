...
use crate::probe::catalog::{AudioPlan, ResourceCatalog};
...
impl<L: AssetLoader, S: JsContext> Pipeline for DefaultPipeline<L, S> {
    ...
    fn render_frame(&mut self, _idx: u32) -> Result<(DrawOpFrame, FrameMediaPlan)> {
        unimplemented!(
            "DefaultPipeline::render_frame is not yet implemented. \
             The engine still uses the legacy runtime/session.rs + runtime/pipeline.rs path. \
