...
use opencat_core::scene::gl_transition;
...
fn gl_transition_effect(effect: &GlTransition) -> Result<RuntimeEffect> {
    GL_TRANSITION_EFFECTS.with(|slot| {
        ...
        let key = gl_transition::normalize_gltransition_name(&effect.name);
        ...
        let sksl = effect
            ...
            .ok_or_else(|| anyhow!("GLTransition `{}` has no SKSL source", effect.name))?;
        let runtime_effect = RuntimeEffect::make_for_shader(&sksl, None).map_err(|err| {
            anyhow!(
                "failed to compile GLTransition `{}` as SKSL: {err}\n{sksl}",
                effect.name
            )
        })?;
        slot.insert(key, runtime_effect.clone());
        Ok(runtime_effect)
    })
}
