...
pub(crate) fn ensure_assets_preloaded(
    ...
    crate::resource::fetch::preload_image_sources(&mut session.assets, req.image_sources)?;
    crate::resource::fetch::preload_audio_sources(&mut session.assets, req.audio_sources)?;

    for path in req.video_paths {
        let _ = crate::resource::probe::probe_video(&mut session.assets, &path, &mut session.media_ctx);
    }
