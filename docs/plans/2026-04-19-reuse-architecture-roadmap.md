...
### 2026-04-19 (Round 2 landing)

- P1-1 完成:`item_is_time_variant(DrawScript)` 改为基于命令序列稳定性(实际实现:无条件 false,由 hash 稳定性充当过滤器);`analyze_live_node_item_execution` 基于 `paint_variance` 分流 —— Stable → Direct(接 ItemPictureCache),TimeVariant → FrameLocalPicture 兜底
- P1-2 完成:`item_paint_fingerprint` 对 Video Bitmap 走独立分支,`media_offset_secs` 按 `quantize_pts`(1/10000 秒)量化后参与 hash;`classify_paint` 对 Video 仍返 TimeVariant 不变,避免祖先子树被误标 stable
- profile 对比(showcase-landscape,480 frames):
  - Round 1 末尾:`backend avg ms/frame 101.12`,`draw_script 98.84`,`item_picture_record 81.44`,`item_hit 0.00`
  - Round 2 末尾:`backend avg ms/frame 73.05`,`draw_script 0.00`,`item_picture_record 70.10`,`item_hit 0.00`,`item_miss 0.65`
  - **分析**:draw_script 从 98.84ms 降到 0.00ms 说明路由改变生效;item_miss 从 0 变为 0.65 证明 DrawScript 走了 ItemPictureCache 路径;item_hit 为 0 是因为 showcase 的 DrawScript 每帧读取时变数据导致命令序列变化,属于内容侧特性而非架构问题
- 下一步:Round 3 做 P1-3(Picture → Image 两段式物化)+ P1-4(bounds-aware 粒度选择)。启动前需要先用 video benchmark 验证 P1-2 的暂停段复用收益
