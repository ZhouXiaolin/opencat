...
| `morphSVG`, `d` | SVG path morphing channel; rewrites a `path` node's path data |
...
- `morphSVG` is the canonical property; `d` is its alias.
- Target must be a `type: "path"` node.
- `from` and `to` must be valid SVG path data strings accepted by Skia.
- `from` and `to` must each contain exactly one contour.
- Open paths may only morph to open paths.
- Closed paths may only morph to closed paths.
- Multi-subpath morphing is not supported. Split it into separate `path` nodes and animate each node independently.
- Open/closed conversion is not supported. Author matching topology before applying `morphSVG`.
- Intermediate frames are generated by arc-length resampling and point correspondence.
- Best for coherent single-contour silhouettes, icons, blobs, and strokes.
