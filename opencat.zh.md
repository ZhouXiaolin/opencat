...
使用标准 SVG Tailwind 工具类：
- `stroke-{color}` / `stroke-[#hex]` — 描边色（默认 Black）
- `stroke-0` / `stroke-1` / `stroke-2` — 描边宽度（默认 2）
- `stroke-[n]` — 任意描边宽度
- `fill-{color}` / `fill-[#hex]` — 填充色（默认无）

### 3.5 `path`

SVG 路径节点。渲染一条或多条 SVG path data 字符串，使用独立的 fill/stroke 样式。
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `d` | 是 | SVG path data 字符串 |

与 `icon` 使用相同的 SVG Tailwind 工具类：
- `fill-{color}` / `fill-[#hex]` — 填充色（默认无）
- `stroke-{color}` / `stroke-[#hex]` — 描边色（默认无）
- `stroke-0` / `stroke-1` / `stroke-2` / `stroke-[n]` — 描边宽度

与 `icon` 不同，`path` 没有默认的内建尺寸——通过 `className` 设置 `w`/`h` 或使用布局。
...
| 用 `bg-{color}` / `text-{color}` 给图标着色 | 用 `stroke-{color}` 设置描边色，`fill-{color}` 设置填充色 |
