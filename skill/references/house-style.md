...
4. **选择字体。** 运行 [typography.md](typography.md) 中的字体发现脚本——或选择您已知且符合主题的字体。该脚本拓宽您的选择，但不是唯一来源。
---
...
- **变化是底线。** 缓动、速度、入场方向、交错节奏——每一项在同一场景内不得重复超过两次。全部 `ease-out` + 全部 0.4 秒 + 全部从下方入场 = 机器人做的东西。
...
- **入场比退场长。** 出现常用 0.4 秒，消失常用 0.23-0.27 秒。人对事物消失的耐心比出现短。
